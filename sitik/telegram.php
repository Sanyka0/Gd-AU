...
// Токен
  const TOKEN = '6494767956:AAF6fz2b2SDjGz_iW5jRDOkkYNnheei7iJw';

  // ID чата
  const CHATID = '-956906241';
  ...
  $types = array('image/gif', 'image/png', 'image/jpeg', 'application/pdf', 'application/zip', 'audio/aac');
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (!empty($_POST['name']) && !empty($_POST['phone']) && !empty($_POST['text'])) {
    ...
    if (isset($_POST['phone']) && !empty($_POST['phone'])) {
        $txt .= "Телеграм: " . strip_tags(trim(urlencode($_POST['phone']))) . "%0A";
    }
    
    if (isset($_POST['text']) && !empty($_POST['text'])) {
      $txt.= "Контент:" . strip_tags(trim(urlencode($_POST['text']))) . "%0A";
    }
